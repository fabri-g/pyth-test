#!/usr/bin/env bash

# This is an example payload for subitting a price update from Hermes to a NEAR contract.
UPDATE_ARGS=$(
	cat <<-EOF
		{
		    "data": ""
		}
	EOF
)

# Feed through jq to get compressed JSON to avoid CLI weirdness.
UPDATE_JSON=$(echo "$UPDATE_ARGS" | jq -c '.' -M)

# Submit.
near contract call-function as-transaction pyth-oracle.testnet update_price_feeds json-args "$UPDATE_JSON" prepaid-gas '300 Tgas' attached-deposit '1 NEAR' sign-as pyth-test.testnet network-config testnet sign-with-legacy-keychain send